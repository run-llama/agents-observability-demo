...
class OtelTracesSqlEngine:
    def __init__(self, engine: Optional[Engine] = None, engine_url: Optional[Dict[str, Any]] = None, table_name: Optional[str] = None, service_name: Optional[str] = None):
    ...
    def _to_sql(self, dataframe: pd.DataFrame, if_exists_policy: Optional[Literal["fail", "replace", "append"]] = None) -> None:
        if not self._connection:
            self._connect()
        dataframe.to_sql(name=self.table_name, con=self._engine, if_exists=if_exists_policy or "replace")

    def to_sql_database(self, start_time: Optional[int] = None, end_time: Optional[int] = None, limit: Optional[int] = None, if_exists_policy: Optional[Literal["fail", "replace", "append"]] = None) -> None:
    ...
    def execute(self, statement: str, parameters: Optional[Union[Dict[str, Any], List[Dict[str, Any]]]] = None, execution_options: Optional[Dict[str, Any]] = None) -> Result:
        if not self._connection:
            self._connect()
        return self._connection.execute(statement=statement, parameters=parameters, execution_options=execution_options)
